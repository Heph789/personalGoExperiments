...
func run3(pass *analysis.Pass) (interface{}, error) {
	...
	inspect.Preorder(nodeFilter, func(node ast.Node) {
		...
		obj := pass.TypesInfo.ObjectOf(i.Name)
		if obj != nil {
			debug.log(node, 542, 542, "", "%v\n", obj)
		}
		// if i.Recv != nil {
...
func run5(pass *analysis.Pass) (interface{}, error) {
	...
	inspect.Preorder(nodeFilter, func(node ast.Node) {
		...
		if callH.identifyFuncLitBlock(expr.Fun) != nil {
			debug.log(node, 565, 565, "", "Found func lit at %v\n", debug.getPosition(node))
		}
		// if lineNumber > 554 && lineNumber < 558 {
